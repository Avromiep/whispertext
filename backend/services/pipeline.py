...
class DictationPipeline:
    ...
    def _is_hallucination(text: str, audio) -> bool:
        ...
        if detected >= MIN_FILLER_SPEECH_S:
            return False
        # Log the length, not the words — logs must never carry dictated text.
        log.info("Discarding likely silence hallucination (%d chars, %.2fs of speech)",
                 len(text), detected)
        return True
    ...
    def _apply_formatting(text: str) -> str:
        ...
        text = text.strip()
        if f.remove_fillers:
            # Deterministic — strips "um"/"uh"/etc. even with AI cleanup off.
        ...
        # smart_format (always on) turns "a quarter"/"a third" into 0.25/0.333 —
        # spell the common fractions back out. Independent of numbers_as_digits.
